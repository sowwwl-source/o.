...
- **deploy-frontend.yml** - Automated frontend deployment to Netlify
...
cp scripts/nginx.conf.example /etc/nginx/sites-available/sowwwl-api
...
| `NETLIFY_AUTH_TOKEN` | From Netlify settings | `nfp_xxx...` |
| `NETLIFY_SITE_ID` | From Netlify site | `abc123...` |
...
└──────┬──────┘
       │
       ├─── Frontend (Netlify) ─── index.html, login.html
       │
       └─── /api/* proxy
              │
              ▼
...
3. **Frontend Deploy**: Deploy static files to Netlify
